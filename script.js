...
function start() {
  ...
  const isNumCourtsValid =
    numCourtInput === "" || isValidNum(numCourts, MAX_COURTS);

  if (!isNumPlayersValid || !isNumCourtsValid) {
    setInputValidation(
      isNumPlayersValid,
      document.getElementById("people-validation"),
      MAX_PLAYERS,
    );
    setInputValidation(
      isNumCourtsValid,
      document.getElementById("court-validation"),
      MAX_COURTS,
    );
    return;
  } else {
...
function updateDirectionButtons() {
  ...
  showPreviousButton.style.visibility =
    currentRound === 1 ? "hidden" : "visible";
  showNextButton.style.visibility =
    currentRound === roundHistory.length ? "hidden" : "visible";
}

function nextRound() {
  const maxPlayersAllowed = Math.min(
    courts * 4,
    Math.floor(players.length / 4) * 4,
  );
  let splitPlayers = [[], []];
  currentRound++;
...
function pushPlayersIntoPlayingOrNot(
  cutoff,
  customPlayers,
  playing,
  notPlaying,
...
function addPlayer() {
  ...
  if (players.length === MAX_PLAYERS) {
    document.getElementById("add-player-validation").innerText =
      `Cannot have more than ${MAX_PLAYERS} players!`;
  ...
  document.getElementById("add-player-confirmation").innerText =
    `New player added! New player is number: ${maxPlayerId}`;
}
...
function removePlayer() {
  ...
  if (
    !isValidNum(playerId, maxPlayerId) ||
  ...
  document.getElementById("remove-player-confirmation").innerText =
    `Player removed! Removed player number: ${playerId}`;
}
...
function ShowHideOptions() {
  ...
  clearDisplayedMessages();
  optionsToggle.innerHTML =
    optionsToggle.innerHTML === SHOW_OPTIONS ? HIDE_OPTIONS : SHOW_OPTIONS;
